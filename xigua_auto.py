...
def run_w(playwright: Playwright,video:dict,state_storage:str) -> None:
...
def run_h(playwright: Playwright,video:dict,state_storage:str) -> None:
    browser = playwright.chromium.launch(headless=False)
    context = browser.new_context(storage_state=state_storage)
    page = context.new_page()
    page.goto("https://studio.ixigua.com/upload")
...
with sync_playwright() as playwright:
    video = dict(
        name='什么都不是',
        url='./data/kuaishou/3xpeki6fwm6uvbc.mp4',
        cover_url='./data/kuaishou/3xpeki6fwm6uvbc.jpeg',
        topic='网海钩陈'
    )
    run_w(playwright,video,state_storage='./auth/state.json')
